...
void dijkstra(grafo *grafoG, vertice *raiz) {
    LIST_HEAD(lista, vertice) lista;
    LIST_INIT(&lista);
    raiz->custo = 0;
    raiz->estado = VERTICE_EM_V1;
    ...
    vizinho *vizinhoIt;
    while (!LIST_EMPTY(&lista)) {
        // Substituir por lógica de escolher o vértice com custo menor.
        verticeIt = lista.lh_first;
        LIST_FOREACH(vizinhoIt, &verticeIt->vizinhos, entradas) {
            if (vizinhoIt->verticeRef->estado == VERTICE_EM_V0) {
                vizinhoIt->verticeRef->pai = verticeIt;
                vizinhoIt->verticeRef->custo =
                    verticeIt->custo + vizinhoIt->peso;
                vizinhoIt->verticeRef->estado = VERTICE_EM_V1;
                LIST_INSERT_AFTER(verticeIt, vizinhoIt->verticeRef,
                                  entradasTmp);
            } else if (vizinhoIt->verticeRef->estado == VERTICE_EM_V1) {
                if (verticeIt->custo + vizinhoIt->peso <
                    vizinhoIt->verticeRef->custo) {
                    vizinhoIt->verticeRef->pai = verticeIt;
                    vizinhoIt->verticeRef->custo =
                        verticeIt->custo + vizinhoIt->peso;
